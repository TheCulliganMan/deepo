[![CircleCI](https://circleci.com/gh/TheCulliganMan/deepo/tree/master.svg?style=svg)](https://circleci.com/gh/TheCulliganMan/deepo/tree/master)
...
#### Step 2. Obtain the all-in-one image from [Docker Hub](https://hub.docker.com/r/theculliganman/machine-learning-base)

```bash
docker pull theculliganman/machine-learning-base
```
...
```bash
nvidia-docker run --rm theculliganman/machine-learning-base nvidia-smi
```
...
```bash
nvidia-docker run -it theculliganman/machine-learning-base bash
```

If you want to share your data and configurations between the host (your machine or VM) and the container in which you are using Deepo, use the -v option, e.g.
```bash
nvidia-docker run -it -v /host/data:/data -v /host/config:/config theculliganman/machine-learning-base bash
```
...
```bash
nvidia-docker run -it --ipc=host theculliganman/machine-learning-base bash
```
...
```bash
docker pull theculliganman/machine-learning-base:cpu
```
...
Now you can try this command:
```bash
docker run -it theculliganman/machine-learning-base:cpu bash
```

If you want to share your data and configurations between the host (your machine or VM) and the container in which you are using Deepo, use the -v option, e.g.
```bash
docker run -it -v /host/data:/data -v /host/config:/config theculliganman/machine-learning-base:cpu bash
```
This will make `/host/data` from the host visible as `/data` in the container, and `/host/config` as `/config`. Such isolation reduces the chances of your containerized experiments overwriting or using wrong data.
...
```bash
docker run -it --ipc=host theculliganman/machine-learning-base:cpu bash
```
...
Note that `docker pull theculliganman/machine-learning-base` mentioned in [Quick Start](#Quick-Start) will give you a standard image containing all available deep learning frameworks. You can customize your own environment as well.
...
```bash
docker pull theculliganman/machine-learning-base:tensorflow
```
...
```bash
docker pull theculliganman/machine-learning-base:all-py36-jupyter
```
...
```bash
nvidia-docker run -it -p 8888:8888 --ipc=host theculliganman/machine-learning-base:all-py36-jupyter jupyter notebook --no-browser --ip=0.0.0.0 --allow-root --NotebookApp.token= --notebook-dir='/root'
```
...
Deepo is [MIT licensed](https://github.com/theculliganman/machine-learning-base/blob/master/LICENSE).
